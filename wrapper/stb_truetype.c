...
/*
todo:
	analytical filtering (LUT)
	shader-based subpixel placement
...
EXPORT FT_Face FTW_LoadFont(char* a,int sz){
	stbtt_fontinfo* info;
	int ofs=stbtt_GetFontOffsetForIndex(a,0);
	if(ofs<0){return NULL;}
	info=STBTT_malloc(sizeof(stbtt_fontinfo),NULL);
	if(!stbtt_InitFont(info,a,ofs)){
