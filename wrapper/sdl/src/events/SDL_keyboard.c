...
static void SDL_SetInputEventText(char* s_target,const char* s,int lg_target){
	int lg=SDL_strlen(s);
	if(lg<lg_target){
		SDL_utf8strlcpy(s_target, s, lg_target);
	}else{
		char* s2=SDL_strdup(s);
		s_target[0]=0xff;
		SDL_memcpy(s_target+1,&s2,sizeof(char*));
		s_target[9]=0;
	}
}
...
SDL_SendKeyboardText(const char *text)
{
    ...
    if (SDL_GetEventState(SDL_TEXTINPUT) == SDL_ENABLE) {
        ...
        SDL_SetInputEventText(event.text.text, text, SDL_arraysize(event.text.text));
        posted = (SDL_PushEvent(&event) > 0);
    }
    return (posted);
}
